...
class RateMyProfApi:
    def __init__(self, school_id: str = "153", testing: bool = False):
...
if __name__ == '__main__':
    ...
    rmps = RateMyProfApi(153)
    with open("ratemyprofs.txt", "r") as file:
        lines = file.readlines()
        with open("out.txt", "w") as csvfile:
            for line in lines:
                result = line.index('tid') + 6
                result2 = line.index('tNumRatings') - 3
                num = line[result:result2]
                reviewslist = rmps.create_reviews_list(num)
                ...
                writer = csv.DictWriter(csvfile, fieldnames=csv_columns)
                #writer.writeheader()
                for data in reviewslist:
                    writer.writerow(data)
                csvfile.write("***{{{")
                csvfile.write("\n")
        """ reviewslist = rmps.create_reviews_list(2668323)
        print(reviewslist)
        rmps.WriteReviewsListToCSV(reviewslist, 2668323) """
